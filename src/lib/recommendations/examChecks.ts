...
import { hasEnglishEvidence } from './profileAdvice.ts';
...
export function checkEnglishExams(
  ...
  if (alternatives) {
    ...
    if (hasEnglishEvidence(profile)) {
      unknowns.push(
        'Сертификат английского указан в анкете. Нужно уточнить соответствие формулировке каталога — Aurora шкалы не пересчитывает.',
      );
      return { why, gaps, unknowns };
    }
  ...
  if (ieltsMin != null) {
    ...
    } else if (hasEnglishEvidence(profile)) {
      unknowns.push('Есть результат английского теста. Соответствие именно этому порогу IELTS по каталогу нужно сверить отдельно.');
    } else {
