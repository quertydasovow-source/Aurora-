...
export function checkBudget(program: RawProgram, profile: Profile): {
  ...
  return {
    ...
    withinBudget: within,
    note: within
      ? `Стоимость ${amount.toLocaleString('ru-RU')} ₸ за учебный год в пределах твоего бюджета.`
      : `Стоимость ${amount.toLocaleString('ru-RU')} ₸ за учебный год выше твоего бюджета.`,
  };
}
