...
const FIELDS = ['it', 'engineering', 'business_management', 'international_relations', 'education_languages', 'natural_sciences', 'psychology'];
...
const PROMPT_VERSION = 'aurora-recs-v4';
...
function rulesOnlyRecommendations(filtered, message, groqErrorKind = 'none') {
  ...
  return {
    ...
    recommendations,
    preview,
    related: (filtered.related || []).map((candidate) => toPublicItem(candidate)),
    nextAction: top
      ? {
...
app.post('/api/profile-advice', async (req, res) => {
  const sanitized = sanitizeProfile(req.body?.profile);
  if (sanitized.error) {
    return res.status(400).json({ notice: sanitized.error });
  }
  const profile = sanitized.profile;
  const incoming = Array.isArray(req.body?.items) ? req.body.items.slice(0, 4) : [];
  ...
  const diagnostics =
    req.body?.diagnostics && typeof req.body.diagnostics === 'object'
      ? {
          strengths: Array.isArray(req.body.diagnostics.strengths) ? req.body.diagnostics.strengths.slice(0, 8).map(String) : [],
          gaps: Array.isArray(req.body.diagnostics.gaps) ? req.body.diagnostics.gaps.slice(0, 8).map(String) : [],
          thingsToClarify: Array.isArray(req.body.diagnostics.thingsToClarify)
            ? req.body.diagnostics.thingsToClarify.slice(0, 8).map(String)
            : [],
        }
      : undefined;
  ...
  const asRules = () =>
    res.json({ mode: 'rules', intro: fallback.intro, suggestions: fallback.suggestions });

  if (!groqClient) return asRules();
  ...
  try {
    ...
    for (const model of models) {
      try {
        ...
        parsed = parseAdviceJson(json, profile);
        if (parsed) break;
      } catch (error) {
        const recovered = extractFailedGeneration(error);
        if (recovered) {
    ...
    if (!parsed) return asRules();
    return res.json({ mode: 'ai', intro: parsed.intro, suggestions: parsed.suggestions });
  } catch (error) {
